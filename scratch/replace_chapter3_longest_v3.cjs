...
\\begin{itemize}
    \\item \\textbf{F3.1 Lexical Winnowing Plagiarism Check:} The background pipeline must check note originality. The system extracts raw text from the PDF using \\texttt{pdf-parse}, converts the text into a normalized string (lowercase, alphanumeric characters only), and splits it into $k$-grams ($k=20$). The algorithm hashes these $k$-grams and selects a subset of min-hashes using a sliding window of size $w=15$. These hashes are queried against the \\texttt{note\_shingles} database index. If the overlap coefficient with any existing note shingle set exceeds 40\\%, the upload is flagged as a duplicate and rejected.
...
`;
  ...
  console.log('Successfully replaced Chapter 3 with super-expanded version v3 in Project Stage II Report.tex');
} catch (err) {
